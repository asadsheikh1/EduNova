...
class LikesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final lStream = ModalRoute.of(context)!.settings.arguments as List<dynamic>?;

    return Scaffold(
      appBar: AppBar(
        backgroundColor: Theme.of(context).appBarTheme.backgroundColor,
        title: Text(AppLocalizations.of(context)!.likedBy),
        elevation: 0,
      ),
      body: Column(
                    ...
                    return Column(
                      ...
                      children: filteredUsers.map((user) {
                        ...
                        return GestureDetector(
                          onTap: () {
                            Navigator.of(context).pushNamed(UserChannelScreen.routeName, arguments: {'email': like['email']});
                          },
                          child: StreamBuilder(
                            stream: Repository.databaseUser.onValue,
                            builder: (BuildContext context, AsyncSnapshot<DatabaseEvent> snapshot) {
                              if (snapshot.hasData) {
                                ...
                                data.forEach((key, value) {
                                  if (value['email'].toString() == like['email']) {
